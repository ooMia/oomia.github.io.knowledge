...
이번 갱신에서 engine `main`과 `develop`은 `6ba2f950a78eef18c2efa305b96a1c8d0443252e`로 동일함을 확인했다. explicit Publish 구현은 `main`에 존재한다. generated docs와 site consumer의 최신 revision은 각각 2026-09-13의 `50d89a4...`와 `a3b2e18...`에 머물러 있지만, 이는 publish 입력에 production content delta가 없었기 때문이다. `docs:publish`는 실제 diff가 있을 때만 docs/site/engine commit을 만들도록 구현되어 있으므로 동일 snapshot에서 revision이 유지되는 idempotent no-op은 정상 성공 semantics다. Issue #8은 이 기준으로 완료 처리했다.
...
| Automation | **충족** | Payload Admin의 명시적 `사이트 발행` action이 `POST /api/publish`를 통해 기존 `vp run docs:publish`를 호출한다. 중복 실행 거부와 non-zero 실패 전파가 테스트되었고 feature branch 실동작은 실제 `docs:publish`의 main-only guard까지 도달했다. `docs:publish`는 content delta가 있을 때만 revision을 갱신하므로 동일 snapshot의 idempotent no-op도 정상 publishing 결과다. [Publish action](https://github.com/ooMia/oomia.github.io.engine/blob/6ba2f950a78eef18c2efa305b96a1c8d0443252e/apps/cms-lab/src/components/PublishAction.tsx), [publish adapter](https://github.com/ooMia/oomia.github.io.engine/blob/6ba2f950a78eef18c2efa305b96a1c8d0443252e/apps/cms-lab/src/publish.ts), [docs workflow](https://github.com/ooMia/oomia.github.io.engine/blob/6ba2f950a78eef18c2efa305b96a1c8d0443252e/apps/cms-lab/scripts/docs-workflow.ts), [tests](https://github.com/ooMia/oomia.github.io.engine/blob/6ba2f950a78eef18c2efa305b96a1c8d0443252e/apps/cms-lab/tests/publish.test.ts), [Issue #8](https://github.com/ooMia/oomia.github.io.engine/issues/8) | 현재 1.0 boundary의 최소 triggered automation 요구는 충족한다. scheduled/agent-driven orchestration은 후속 확장 범위다. |
...
1. **Authoring:** 1.0에서 요구할 작성 표현 범위와 preview/draft-public 경계를 확정하고 해당 범위를 완료.
2. **Extensibility:** 현재 Callout proof를 일반화한 명시적 extension contract.
3. **Delivery:** verified docs → site commit → GitHub Pages의 실제 성공 deployment Evidence.
4. **Release gate:** 1.0 public contract 목록, compatibility policy, 최종 release gate AC.
